...
def main(subject_id, date, block):   
    ...
    print(os.getcwd())
    char_dir = 'Block_2/characters/'
    scene_dir = 'Block_2/encoding/'
    item_dir = 'Block_2/individual_items/'
    
    
    # Define lists, dataframes, etc. for stimuli
    scene_images = [i for i in os.listdir('./Block_2/encoding/') 
                    if i.endswith('.psd')]
    stimulus_key = pd.read_csv('Block_2/stimulus_key.csv')
    stimulus_key['Stim'] = stimulus_key['Item'] + '_' + stimulus_key['Color']
    character_list = stimulus_key['Character'].unique().tolist()
    random.shuffle(character_list)
    fixation = 'Block_2/fixation_cross.png'
    ...
    save_subj_file_name = data_dir+subject_id+'_'+date+'_block2.csv'
