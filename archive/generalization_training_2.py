...
# Set stimulus timing and trial data
beta = 1
if beta == 0:
    time_intro = 3
    time_encod = 5
...
#def main(subject_id, date, block):   
print(subject_id)
print(date)
print(block)
# Set relevant paths
...
char_dir = 'Training'+'/characters/'
scene_dir = 'Training'+'/encoding/'
item_dir = 'Training'+'/individual_items/'


# Define lists, dataframes, etc. for stimuli
scene_images = [i for i in os.listdir('./Training'+'/encoding/') 
                if i.endswith('.png')]
stimulus_key = pd.read_csv('Training'+'/stimulus_key.csv')
stimulus_key['Stim'] = stimulus_key['Item'] + '_' + stimulus_key['Color']
character_list = stimulus_key['Character'].unique().tolist()
random.shuffle(character_list)
fixation = 'Training'+'/fixation_cross.png'
subject_stim = pd.read_csv('Training'+'/subject_stim.csv')
#subject_stim = pd.DataFrame(columns=['Part','Character','Scene','Item','Color',
#                                     'Lure_1','Lure_2','Answer','Reaction_Time'])
r = open('training_instructions.txt', encoding='utf8')
...
for character in character_list:
    subject_items = []
    count = 0
    encoding_character_order = [1, 2, 3, 4]
    ...
    temp_instr = visual.TextStim(win, instr[1], color='black', pos=[0,0.7])
    temp_instr.draw()
    #char_stim = char_dir+character+'.png'
    ...
    char_pres = visual.ImageStim(win, char_stim, pos=[-0.5,0])
    ...
    ### Start encoding
    temp_instr = visual.TextStim(win, instr[2], color='black', pos=[0,0])
    temp_instr.draw()
    win.update()
    event.waitKeys(keyList=['space'])
    win.flip()
    ...
    for n in range(4):
        if event.getKeys(['escape']):
            win.close()
            core.quit()
        subj_scene = subject_stim[subject_stim['Character']==character]['Scene'].iloc[n]
        scene_stim = scene_dir+subj_scene+'.png'
    ...
    temp_instr = visual.TextStim(win, instr[3], color='black', pos=[0,0])
    ...
    gen_lure_items.loc['games'] = ['pillow','basket']
    gen_lure_items.loc['veges'] = ['sand_castle','trashcan']
    gen_lure_items = subject_stim[subject_stim['Part']=='generalization'][['Character','Lure_1','Lure_2']]
    ...
    #lure1 = gen_lure_items[gen_lure_items['Character']==character]['Lure_1'].iloc[0]
    #lure2 = gen_lure_items[gen_lure_items['Character']==character]['Lure_1'].iloc[1]
    ...
    target = subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='generalization')]['Item'].iloc[0] + '_' + subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='generalization')]['Color'].iloc[0]
    lure1 = subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='generalization')]['Lure_1'].iloc[0]
    lure2 = subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='generalization')]['Lure_2'].iloc[0]
    ...
    char_stim = Image.open(char_dir+
                           [i for i in os.listdir(char_dir)
                           if i.endswith(character+'.png')][0])
    lure1_stim = Image.open(item_dir + '/' + 
                           [i for i in os.listdir(item_dir+'/')
                           if i.startswith(lure1+'.png')][0])
    lure2_stim = Image.open(item_dir + '/' + 
                           [i for i in os.listdir(item_dir+'/')
    ...
    scene_cb = subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='context_binding')]['Scene'].iloc[0]
    target_cb = subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='context_binding')]['Item'].iloc[0] + '_' + subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='context_binding')]['Color'].iloc[0]
    lure1_cb = subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='context_binding')]['Lure_1'].iloc[0]
    lure2_cb = subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='context_binding')]['Lure_2'].iloc[0]
    context_bind_items = context_bind_items + [target_cb.rsplit('_',1)[0], 
                                               lure1_cb.rsplit('_',1)[0], 
                                               lure2_cb.rsplit('_',1)[0]]
    ...
    target_ir = subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='item_recognition')]['Item'].iloc[0]
    lure1_ir = subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='item_recognition')]['Lure_1'].iloc[0]
    lure2_ir = subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='item_recognition')]['Lure_2'].iloc[0]
    ...
    #lure1_ps = lures_ps[0]
    #lure2_ps = lures_ps[1]
    target_ps = subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='pattern_separation')]['Item'].iloc[0] + '_' + subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='pattern_separation')]['Color'].iloc[0]
    lure1_ps = subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='pattern_separation')]['Lure_1'].iloc[0]
    lure2_ps = subject_stim[(subject_stim['Character']==character) & 
                          (subject_stim['Part']=='pattern_separation')]['Lure_2'].iloc[0]
