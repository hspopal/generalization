...
def main(subject_id, date):   
    ...
    print(os.getcwd())
    char_dir = 'Block_1/characters/'
    scene_dir = 'Block_1/encoding/'
    item_dir = 'Block_1/individual_items/'
    
    # Define lists, dataframes, etc. for stimuli
    scene_images = [i for i in os.listdir('./Block_1/encoding/') 
                    if i.endswith('.psd')]
    stimulus_key = pd.read_csv('Block_1/stimulus_key.csv')
    stimulus_key['Stim'] = stimulus_key['Item'] + '_' + stimulus_key['Color']
    character_list = stimulus_key['Character'].unique().tolist()
    random.shuffle(character_list)
    fixation = 'Block_1/fixation_cross.png'
    ...
    scored_answers = pd.DataFrame(columns=['Subject_ID','Date','G_Acc','CB_Acc','IR_Acc',
                              'PS_Acc'])
    ...
    if beta == 0:
        ...
        if os.path.exists('subject_data/dev_testing/all_data.csv'):
            scored_answers.to_csv('subject_data/dev_testing/all_data.csv', mode='a', header=False, 
                                  index=False)
        else:
            scored_answers.to_csv('subject_data/dev_testing/all_data.csv', mode='a', index=False)
